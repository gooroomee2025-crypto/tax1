const progressStore = {}; // chapterId -> {answeredCount, correctCount, done, wrongIds, completionCount}
...
function emptyProgress(){
  return { answeredCount: 0, correctCount: 0, done: false, wrongIds: [], completionCount: 0 };
}
...
function renderPartBlocks(){
  ...
  wrap.innerHTML = PARTS.map(part => `
    <div class="part-block">
      ...
      <div class="empty-note">다른 챕터는 문제 자료를 주시면 이어서 추가할 수 있어요.</div>
    </div>
  `).join('');
          ...
          <div class="wrongnote-chapter-body">
            ${wrongQs.map(q => `
              <div class="wrongnote-item">
                ...
                <div class="wq-answer">정답: ${q.answer}</div>
              </div>
            `).join('')}
          </div>
        </div>
...
async function finishQuiz(){
  ...
  if(state.isFullRun && state.currentChapterId){
    ...
    await saveProgress(state.currentChapterId);
  }
